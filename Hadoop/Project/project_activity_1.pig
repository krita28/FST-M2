-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/swatthot/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/swatthot/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/swatthot/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/swatthot/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/swatthot/outputs' USING PigStorage('\t');